...
public class Solution_Graph {
    ...
    public boolean NUm391isRectangleCover(int[][] rectangles) {
        ...
        for (int[] rec : rectangles) {
            ...
            map.put(bottomleft, map.getOrDefault(bottomleft, 0) + 1);
            map.put(bottomleft, map.getOrDefault(bottomleft, 0) + 1);
    ...
    public int Num323againcountComponents(int n, int[][] edges) {
        int[] unions = new int[n];
        for (int i = 0; i < n; i++) {
            unions[i] = i;
        }
        for (int i = 0; i < edges.length; i++) {
            int x = edges[i][0];
            int y = edges[i][1];
            Num323againunion(x, y, unions);
        }
        HashSet<Integer> set = new HashSet<>();
        for (int i = 0; i < n; i++) {
            set.add(Num323againfind(i, unions));
        }
        return set.size();

    }

    public int Num323againfind(int x, int[] unions) {
        if (unions[x] == x)
            return x;
        return Num323againfind(unions[x], unions);
    }

    public void Num323againunion(int x, int y, int[] unions) {
        int xf = Num323againfind(x, unions);
        int yf = Num323againfind(y, unions);
        if (xf != yf) {
            unions[x] = yf;
        }
    }

    public class Num305uf
    {
        int count;
        int[] parent;

        public Num305uf(int n)
        {
            count = 0;
            parent = new int[n];
    ...
    public List<Integer> Num305numIslands2(int m, int n, int[][] positions) {
        ...
        for(int i=0;i<positions.length;i++)
        {
            ...
            {
                if((r+direc[0]>=0&&r+direc[0]<=m-1)&&(c+direc[1]>=0&&c+direc[1]<=n-1))
                {
                    int adj_index = (r+direc[0])*n+c+direc[1];
                    if(uf.parent[adj_index]!=-1)
                    {
                        uf.union(adj_index,index);
                    }
                }
            }
            ans.add(uf.count);
    ...
    public double[] calcEquation(List<List<String>> equations, double[] values, List<List<String>> queries) {
        ...
        for(int i=0;i<queries.size();i++)
        {
            ...
            {
                HashSet<String> visited = new HashSet<>();
                ans[i]=a399helper(map,a,b,1,visited);
            }
        }
        return ans;
    ...
    public double a399helper(HashMap<String,HashMap<String,Double>> map,String a, String b,double currentval, HashSet<String> visited)
    {
        if(a==b)
        {
            return currentval;
        ...
        double res = -1;
        visited.add(a);
        HashMap<String, Double> neighs = map.get(a);
        for(String next: neighs.keySet())
        {
            if(visited.contains(next))
                continue;
            res  = a399helper(map,next,b,currentval*neighs.get(next),visited);
            if(res!=-1)
                break;
        }
        visited.remove(a);
    ...
    public int Num1334findTheCity(int n, int[][] edges, int distanceThreshold) {
    ...
    public boolean Num1153canConvert(String str1, String str2) {
        if (str1.equals(str2))
            return true;
        ...
        if(map.size()==26&&set.size()==26)
            return false;
        return true;
    }

    public int[] Num1203sortItems(int n, int m, int[] group, List<List<Integer>> beforeItems) {
        ...
        List<Integer> grouplist = topsort(groupAdj,groupindegree,m);
        if(grouplist.size()==0)
            return new int[0];
        List<Integer> itemlist = topsort(itemadj,itemindegree,n);
        if(itemlist.size()==0)
            return new int[0];
        ...
        List<Integer> ans = new ArrayList<>();
        for(int gp:grouplist)
        {
            List<Integer> items = map.getOrDefault(gp,new ArrayList<>());
            ans.addAll(items);
        }
        return ans.stream().mapToInt(Integer::valueOf).toArray();
    }

    public List<Integer> topsort(List<Integer>[] adjlist,int[] indegreelist, int totalnum)
    {
        List<Integer> ans = new LinkedList<>();
        Queue<Integer> queue = new LinkedList<>();
    ...
    int num1191rank = 0;
    List<List<Integer>> Num1192result = new LinkedList<>();
    public List<List<Integer>> Num1192criticalConnections(int n, List<List<Integer>> connections) {
        ...
        Arrays.fill(low,-1);
        Arrays.fill(dfn,-1);
        Num1192helper(low,dfn,graph,0,-1);
        return Num1192result;
    }

    public void Num1192helper(int[] low, int[] dfn, List<Integer>[] graph,int current,int parent)
    {
        if(dfn[current]!=-1)
            return;
        low[current]= dfn[current] = num1191rank++;
        for(int next:graph[current])
        {
            if(next==parent)
                continue;
            Num1192helper(low,dfn,graph,next,current);
            if(dfn[current]<low[next])
            {
                Num1192result.add(Arrays.asList(current,next));
            }
            else
            {
